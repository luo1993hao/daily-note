...
##### 描述
- 支持崩溃恢复的原子广播协议
- Zk依赖zab实现分布式数据一致性。达到了集群中各副本数据间的一致性
##### 主要流程
- 事务请求->leader转换为proposal->分发给follower->半数follower正确反馈->leader再次发送commit->follower提交
##### 细节
- 崩溃恢复
- 消息广播
  - 简化版二阶段提交（中断逻辑移除，过半follower反馈）
  - 无法处理leader崩溃带来的数据不一致=>崩溃恢复
- 数据同步
  - 每个follower一个队列，未同步的事务=>同步后加入可用follower列表
  - zxid
     - 高32位为leader epoch编号（每次选举+1）
     - 低32位递增的计数器
 #### 场景
...
##### 大概流程
  ...
  - 处理投票
    -zxid->myid 票形：（sid,zxid）
      - 原因说明：
         - 场景1：leader->follower->commit过程挂掉=>确保已经在leader服务器提交的事务最终被所有服务器提交
         - 场景2：leader提出p1后挂掉=>当leader恢复后，要保证丢弃p1
         - 前提:zxid是递增的，所以ZK leader选举的时候直接选择zxid最大值。可以省去检查proposal的提交和丢弃
  - 每个服务器统计投票
  - 改变状态
##### 时机
- 服务器初始化
...
- 事务日志
  ...
  - 64mb,文件名后缀:zxid
  - 写入时，磁盘空间预分配
     - <4Kb，文件大小变为64mb。使用0填充（避免磁盘seek(开辟新的io快)）
- 日志快照
  - dataDir
  - 没有预分配。文件名后缀:zxid
  - 时机：过半策略
#### 运维
- 凡是zk数据状态变更，都以事务日志的形式写入磁盘，只有集群中过半的服务器记录后，才会给客户端反应。
  - 单独的磁盘作为事务
##### 容灾
- 三机房部署
  - 总数为N
